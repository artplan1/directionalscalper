...
from directionalscalper.core.strategies.bybit.bybit_strategy_async import BybitStrategyAsync
...
class LinearGridBaseFuturesSingle(BybitStrategyAsync):
    def __init__(
        self, exchange: 'BybitExchangeAsync', manager, config, symbols_allowed=None, mfirsi_signal=None
    ):
    ...
    async def run(self, symbol, open_symbols={}, mfirsi_signal=None, action=None):
        try:
            standardized_symbol = symbol.upper()
            logging.info(f"Standardized symbol: {standardized_symbol}")

            if action == "long":
                await self.run_long_trades(standardized_symbol, mfirsi_signal, open_symbols=open_symbols)
            elif action == "short":
                await self.run_short_trades(standardized_symbol, mfirsi_signal, open_symbols=open_symbols)
        except Exception as e:
            logging.error(f"Exception in run function: {e}")
            logging.debug(traceback.format_exc())

    async def run_long_trades(self, symbol, mfirsi_signal=None, open_symbols={}):
        if self.running_long is None:
            self.running_long = True

        await self.run_single_symbol(symbol, mfirsi_signal, "long", open_symbols=open_symbols)

    async def run_short_trades(self, symbol, mfirsi_signal=None, open_symbols={}):
        if self.running_short is None:
            self.running_short = True

        await self.run_single_symbol(symbol, mfirsi_signal, "short", open_symbols=open_symbols)
    ...
    async def configure_trader(self, symbol):
        ...
        await self.exchange.setup_exchange(symbol)
    ...
    async def run_single_symbol(
        ...
        try:
            ...
            if (
                ...
                # Fetch equity data
                fetched_total_equity = await self.retry_api_call_async(self.exchange.get_futures_balance_async)
            ...
            if not available_equity:
                ...
                available_equity = await self.retry_api_call_async(self.exchange.get_available_balance_async)
            ...
            funding_check = await self.is_funding_rate_acceptable(symbol)
            ...
            position_data = await self.retry_api_call_async(self.exchange.get_positions_async, symbol)
            ...
            try:
                ...
                if await self.check_position_inactivity_async(
                    symbol,
                ...
                logging.info(f"Exception caught in check_position_inactivity_async {e}")
            ...
            if not self.running_long and not self.running_short:
                ...
                self.active_long_grids.discard(symbol)
                self.active_short_grids.discard(symbol)
                await self.cleanup_before_termination_async(symbol)
                logging.info(
            ...
            if symbol in open_symbols or trading_allowed:  # and instead of or
                # Fetch the API data
                api_data = await self.manager.get_api_data(symbol)
                ...
                long_dynamic_amount, short_dynamic_amount = await self.calculate_dynamic_amounts_notional_async(
                    symbol=symbol,
                ...
                long_dynamic_amount_helper, short_dynamic_amount_helper = await self.calculate_dynamic_amounts_notional_nowelimit_async(
                    symbol=symbol,
                    total_equity=total_equity,
                    best_bid_price=best_bid_price,
                    best_ask_price=best_ask_price,
                ...
                await self.liq_stop_loss_logic_async(
                    long_pos_qty,
                    long_pos_price,
                ...
                await self.stop_loss_logic_async(
                    long_pos_qty,
                    long_pos_price,
                    short_pos_qty,
                    short_pos_price,
                ...
                if long_pos_qty > 0:
                    try:
                        unrealized_pnl = await self.exchange.fetch_unrealized_pnl_async(symbol)
