...
class LinearGridBaseFuturesWs(BybitStrategy):
    def __init__(self, exchange, manager, config, symbols_allowed=None, mfirsi_signal=None):
    ...
    def _initialize_symbol_locks(self, symbol):
    ...
    def run(self, symbol, mfirsi_signal=None, action=None):
        try:
            self._initialize_symbol_locks(symbol)
            ...
            if symbol_locks[standardized_symbol][action].acquire(blocking=False):
                ...
                try:
                    if action == "long":
                        self.run_long_trades(standardized_symbol, mfirsi_signal)
                    elif action == "short":
                        self.run_short_trades(standardized_symbol, mfirsi_signal)
                finally:
    ...
    def run_long_trades(self, symbol, mfirsi_signal=None):
        self.running_long = True
        self.run_single_symbol(symbol, mfirsi_signal, "long")

    def run_short_trades(self, symbol, mfirsi_signal=None):
        self.running_short = True
        self.run_single_symbol(symbol, mfirsi_signal, "short")

    def run_single_symbol(self, symbol, mfirsi_signal=None, action=None):
        try:
            ...
            if previous_long_pos_qty > 0 and long_pos_qty == 0:
                ...
                return  # Exit the while loop, thus ending the thread
            ...
            if not self.running_long and not self.running_short:
                ...
                state.shared_symbols_data.pop(symbol, None)  # Remove the symbol from shared_symbols_data
            ...
            if symbol in open_symbols or trading_allowed: # and instead of or
                ...
                if not self.running_long and not self.running_short:
                    logging.info("Both long and short operations have ended. Preparing to exit loop.")
                    sate.shared_symbols_data.pop(symbol, None)  # Remove the symbol from shared symbols data
