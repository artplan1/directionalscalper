...
general_rate_limiter = RateLimitAsync(50, 1)
...
class SingleBot:
    ...
    def __init__(self, config: Config, exchange_name: str, account_name: str):
        ...
        loop.set_default_executor(ThreadPoolExecutor(max_workers=max(self.exchange_config.symbols_allowed, 100) * 2))

    async def run(self):
        # subscribe to ws
        loop = asyncio.get_event_loop()
        asyncio.run_coroutine_threadsafe(self._subscribe_to_ws(), loop)
            ...
            try:
                ...
                if len(trading_symbols) < self.exchange_config.symbols_allowed:
                    if (
                        ...
                        async with general_rate_limiter:
    ...
    def _process_new_trades(self, trades):
        try:
            ...
            if not state.balance.get('total'):
                logging.warning("Empty balance. Skipping signal")
                return
    ...
    async def _run_live_table(self):
        await LiveTableManager().display_table()
    ...
    async def _fetch_updated_symbols(self) -> List[str]:
        ...
        if self.whitelist:
            ...
            potential_symbols = await self.manager.get_auto_rotate_symbols(
    ...
    async def _run_strategy(
        ...
        try:
            ...
            if action == "long":
                await trader.run(symbol, mfirsi_signal=signal, action="long", open_symbols=self.open_position_symbols)
                ...
                await trader.run(symbol, mfirsi_signal=signal, action="short", open_symbols=self.open_position_symbols)
    ...
    async def _fetch_balance(self):
        available_balance, total_balance = await self.exchange.get_balance_async()
        state.balance['total'] = total_balance
        state.balance['available'] = available_balance
        state.balance["updated_at"] = time.time()
