...
class LiveTableManager:
    ...
    def generate_table(self) -> Table:
        ...
        balance = "{:.4f}".format(float(state.balance.get('total', 0)))
        available_bal = "{:.4f}".format(float(state.balance.get('available', 0)))
        ...
        if last_symbol_data:
            ...
            table.caption = f"Balance: {balance} | Available: {available_bal} | Total uPnL: loading... | Updated: {current_time}"
        else:
